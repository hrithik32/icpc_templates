 ...
 // checking previously total taken time and items
 int time_taken=0;
 int item_taken=0;

 for (int i = 0; i < lev ; ++i)
 {
 	if (taken[i])//check it the i'th item is taken
 	{
 		time_taken+=t[i];
 		item_taken++;
 	}
 }
 //we also have to add the current lev (item & time)
 time_taken+=t[lev];
 item_taken++;
