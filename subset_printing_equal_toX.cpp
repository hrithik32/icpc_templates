...
bool rec(int lev ,int left){
	...
	{
		ans=1;
	}else if (rec(lev+1,left-v[lev]))
	{
		ans=1;
	}
	//save & return
	return dp[lev][left]=ans;
}



// printing multiple solution
...
void printset(int level, int left, vector<int> &cur){
    ...
    if(left >= v[level] && rec(level+1, left-v[level])){
        cur.push_back(v[level]);
        printset(level+1, left-v[level], cur);
        cur.pop_back();
    }
}

// for single solution print
