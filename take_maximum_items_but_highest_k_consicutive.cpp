...
int rec(int lev ,int taken,int consi){
	...
	//cache check
	if (dp[lev][taken][consi]!= -1)
	{
		return dp[lev][taken][consi];
	}
	//compute

	//dont take
	int ans=rec(lev+1,taken,0);
	//take
	if (taken+w[lev]<=x && consi+1<=c)
	{
		ans=max(ans, 1+rec(lev+1,taken+w[lev],consi+1));
	}


	//save & return
	return dp[lev][taken][consi]=ans;
}
