...
int n, m, k;
int dp[2005][2005]; // DP table for memoization

// Recursive function to compute the number of ways
int rec(int lev, int breaks) {

    // Base case: if all bricks are processed
    if (lev == n) {
        if(breaks == k){
            return 1;
        }else return 0;
    ...
    // Cache check
    if (dp[lev][breaks] != -1) {
        return dp[lev][breaks];
    }

    // Case 1: Current brick has the same color as the previous brick
    int ans = rec(lev + 1, breaks);

    // Case 2: Current brick has a different color from the previous brick
    if (breaks + 1 <= k) {
        ans = (ans + (m - 1) * rec(lev + 1, breaks + 1)) % MOD;
    }

    // Save and return
    return dp[lev][breaks] = ans;
}
...
void solve() {
    ...
    // The first brick can be colored in M ways
    int result =(m * rec(1, 0)) % MOD;
