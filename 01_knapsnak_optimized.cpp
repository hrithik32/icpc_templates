 ...
 int rec(int lev,int time_taken,int item_taken){
 
 //base case
 if (lev==n)
 {
 	return 0;//maximum min/max prob returns 0
 }

//cache check
 if (dp[lev][time_taken][item_taken]!= -1)
 {
 	return dp[lev][time_taken][item_taken];
 }
// choice don't take the item
int ans=rec(lev+1,time_taken,item_taken);

//choice take the item
if (time_taken+t[lev]<=x && item_taken+1<=k)
 {
 	ans=max(ans, s[lev]+rec(lev+1,time_taken+t[lev],item_taken+1));

 }
 //save & return
 return dp[lev][time_taken][item_taken] = ans;
